  ...
  up: async (queryInterface, Sequelize) => {
    queryInterface.sequelize.transaction((t) =>
        Promise.all(Object.keys(searchObjects).map((table) =>
            queryInterface.sequelize.query(`ALTER TABLE "${table}" ADD COLUMN ${vectorName} TSVECTOR;`, { transaction: t })
                .then(() =>
                    queryInterface.sequelize.query(`
                UPDATE "${table}" SET ${vectorName} = to_tsvector('english', ${searchObjects[table].map((el) => `"${table}"."${el}"`).join(' || " " || ')});
              `, { transaction: t })
                ).then(() =>
                queryInterface.sequelize.query(`
